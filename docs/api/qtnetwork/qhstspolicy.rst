:orphan:

.. sip:class:: PyQt6.QtNetwork.QHstsPolicy
    :description: QtNetwork/QHstsPolicy-c.rst

    .. sip:enum:: PyQt6.QtNetwork.QHstsPolicy.PolicyFlag
        :description: QtNetwork/QHstsPolicy-PolicyFlag-e.rst

        .. sip:enum-member:: PyQt6.QtNetwork.QHstsPolicy.PolicyFlag.IncludeSubDomains
            :description: QtNetwork/QHstsPolicy-PolicyFlag-IncludeSubDomains-v.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.__init__
        :description: QtNetwork/QHstsPolicy-__init__-f.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.__init__
        :args:
            :sip:ref:`~PyQt6.QtNetwork.QHstsPolicy`
        :description: QtNetwork/QHstsPolicy-__init__-f-1.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.__init__
        :args:
            Union[:sip:ref:`~PyQt6.QtCore.QDateTime`, datetime.datetime]
            :sip:ref:`~PyQt6.QtNetwork.QHstsPolicy.PolicyFlag`
            Optional[str]
            mode: :sip:ref:`~PyQt6.QtCore.QUrl.ParsingMode` = :sip:ref:`~PyQt6.QtCore.QUrl.ParsingMode.DecodedMode`
        :description: QtNetwork/QHstsPolicy-__init__-f-4.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.__eq__
        :args:
            :sip:ref:`~PyQt6.QtNetwork.QHstsPolicy`
        :returns:
            bool
        :description: QtNetwork/QHstsPolicy-__eq__-f.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.expiry
        :returns:
            :sip:ref:`~PyQt6.QtCore.QDateTime`
        :description: QtNetwork/QHstsPolicy-expiry-f.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.host
        :args:
            options: :sip:ref:`~PyQt6.QtCore.QUrl.ComponentFormattingOption` = :sip:ref:`~PyQt6.QtCore.QUrl.ComponentFormattingOption.FullyDecoded`
        :returns:
            str
        :description: QtNetwork/QHstsPolicy-host-f-1.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.includesSubDomains
        :returns:
            bool
        :description: QtNetwork/QHstsPolicy-includesSubDomains-f.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.isExpired
        :returns:
            bool
        :description: QtNetwork/QHstsPolicy-isExpired-f.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.__ne__
        :args:
            :sip:ref:`~PyQt6.QtNetwork.QHstsPolicy`
        :returns:
            bool
        :description: QtNetwork/QHstsPolicy-__ne__-f.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.setExpiry
        :args:
            Union[:sip:ref:`~PyQt6.QtCore.QDateTime`, datetime.datetime]
        :description: QtNetwork/QHstsPolicy-setExpiry-f.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.setHost
        :args:
            Optional[str]
            mode: :sip:ref:`~PyQt6.QtCore.QUrl.ParsingMode` = :sip:ref:`~PyQt6.QtCore.QUrl.ParsingMode.DecodedMode`
        :description: QtNetwork/QHstsPolicy-setHost-f-1.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.setIncludesSubDomains
        :args:
            bool
        :description: QtNetwork/QHstsPolicy-setIncludesSubDomains-f.rst

    .. sip:method:: PyQt6.QtNetwork.QHstsPolicy.swap
        :args:
            :sip:ref:`~PyQt6.QtNetwork.QHstsPolicy`
        :description: QtNetwork/QHstsPolicy-swap-f.rst
