:orphan:

.. sip:class:: PyQt6.QtNetwork.QAuthenticator
    :description: QtNetwork/QAuthenticator-c.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.__init__
        :description: QtNetwork/QAuthenticator-__init__-f.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.__init__
        :args:
            :sip:ref:`~PyQt6.QtNetwork.QAuthenticator`
        :description: QtNetwork/QAuthenticator-__init__-f-1.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.__eq__
        :args:
            :sip:ref:`~PyQt6.QtNetwork.QAuthenticator`
        :returns:
            bool
        :description: QtNetwork/QAuthenticator-__eq__-f.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.isNull
        :returns:
            bool
        :description: QtNetwork/QAuthenticator-isNull-f.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.__ne__
        :args:
            :sip:ref:`~PyQt6.QtNetwork.QAuthenticator`
        :returns:
            bool
        :description: QtNetwork/QAuthenticator-__ne__-f.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.option
        :args:
            Optional[str]
        :returns:
            Any
        :description: QtNetwork/QAuthenticator-option-f-1.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.options
        :returns:
            Dict[str, Any]
        :description: QtNetwork/QAuthenticator-options-f.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.password
        :returns:
            str
        :description: QtNetwork/QAuthenticator-password-f.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.realm
        :returns:
            str
        :description: QtNetwork/QAuthenticator-realm-f.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.setOption
        :args:
            Optional[str]
            Any
        :description: QtNetwork/QAuthenticator-setOption-f-1.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.setPassword
        :args:
            Optional[str]
        :description: QtNetwork/QAuthenticator-setPassword-f-1.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.setUser
        :args:
            Optional[str]
        :description: QtNetwork/QAuthenticator-setUser-f-1.rst

    .. sip:method:: PyQt6.QtNetwork.QAuthenticator.user
        :returns:
            str
        :description: QtNetwork/QAuthenticator-user-f.rst
