...
- Python types that are sub-classed from :sip:ref:`~PyQt5.QtCore.QObject` can
...
C++ classes, i.e. using the :sip:ref:`~PyQt5.QtQml.qmlRegisterType`,
:sip:ref:`~PyQt5.QtQml.qmlRegisterSingletonType`,
:sip:ref:`~PyQt5.QtQml.qmlRegisterUncreatableType` and
:sip:ref:`~PyQt5.QtQml.qmlRegisterRevision` functions.
...
Using :sip:ref:`~PyQt5.QtQml.QQmlListProperty`
----------------------------------------------

Defining list-based properties in Python that can be updated from QML is done
using the :sip:ref:`~PyQt5.QtQml.QQmlListProperty` class.  However the way it
is used in Python is slightly different to the way it is used in C++.

In the simple case :sip:ref:`~PyQt5.QtQml.QQmlListProperty` wraps a Python
...
:sip:ref:`~PyQt5.QtQml.QQmlListProperty` also acts as a proxy for the Python
list so that the following can be written::

    for guest in party.guests:
        print("Guest:", guest.name)

:sip:ref:`~PyQt5.QtQml.QQmlListProperty` can also be used to wrap a *virtual*
...
:sip:ref:`~PyQt5.QtQml.qmlRegisterType`.  Instances of both can be created from
QML.  ``slices`` is a property of ``PieChart`` that, as far as QML is
concerned, is a list of ``PieSlice`` instances.

The :sip:ref:`~PyQt5.QtCore.pyqtProperty` decorator specifies that the property
is a :sip:ref:`~PyQt5.QtQml.QQmlListProperty`, that its name is ``slices`` and
that the ``slices()`` function is its getter.

The getter returns an instance of :sip:ref:`~PyQt5.QtQml.QQmlListProperty`.
...
:sip:ref:`~PyQt5.QtQml.QQmlListProperty` instance rather than the ``PyChart``
...
- When calling :sip:ref:`~PyQt5.QtQml.qmlRegisterType` to register a type that
...
- The instance of an attached properties object is retrieved using the
  :sip:ref:`~PyQt5.QtQml.qmlAttachedPropertiesObject` function in the same way
  that you would from C++.  Just like :sip:ref:`~PyQt5.QtQml.qmlRegisterType`,
  :sip:ref:`~PyQt5.QtQml.qmlAttachedPropertiesObject` takes an additional first
...
:sip:ref:`~PyQt5.QtCore.QObject` and
:sip:ref:`~PyQt5.QtQml.QQmlPropertyValueSource` and provide an implementation
of the :sip:ref:`~PyQt5.QtQml.QQmlPropertyValueSource.setTarget` method.


Using :sip:ref:`~PyQt5.QtQml.QQmlParserStatus`
----------------------------------------------
...
:sip:ref:`~PyQt5.QtCore.QObject` and
:sip:ref:`~PyQt5.QtQml.QQmlParserStatus` and provide implementations of the
:sip:ref:`~PyQt5.QtQml.QQmlParserStatus.classBegin` and
:sip:ref:`~PyQt5.QtQml.QQmlParserStatus.componentComplete` methods.
...
plugins are sub-classes of :sip:ref:`~PyQt5.QtQml.QQmlExtensionPlugin`.  PyQt5
...
the implementation of :sip:ref:`~PyQt5.QtQml.QQmlExtensionPlugin`, creating
an instance of that class, and calling the instance's
:sip:ref:`~PyQt5.QtQml.QQmlExtensionPlugin.registerTypes` method.  By default
the ``pyqt5qmlplugin`` is installed in the ``PyQt5`` sub-directory of your Qt
