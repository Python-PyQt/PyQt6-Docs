...
.. image:: ../../../images/windows-pushbutton.png
...
.. literalinclude:: ../../../snippets/qtbase-src-widgets-doc-snippets-code-src_gui_widgets_qpushbutton.py
    :lines: 54-54
...
Command buttons in dialogs are by default auto-default buttons, i.e., they become the default push button automatically when they receive the keyboard input focus. A default button is a push button that is activated when the user presses the Enter or Return key in a dialog. You can change this with :sip:ref:`~PyQt6.QtWidgets.QPushButton.setAutoDefault`. Note that auto-default buttons reserve a little extra space which is necessary to draw a default-button indicator. If you do not want this space around your buttons, call :sip:ref:`~PyQt6.QtWidgets.QPushButton.setAutoDefault`\ (false).
...
As a general rule, use a push button when the application or dialog window performs an action when the user clicks on it (such as Apply, Cancel, Close and Help) *and* when the widget is supposed to have a wide, rectangular shape with a text label. Small, typically square buttons that change the state of the window rather than performing an action (such as the buttons in the top-right corner of the :sip:ref:`~PyQt6.QtWidgets.QFileDialog`) are not command buttons, but tool buttons. Qt provides a special class (\ :sip:ref:`~PyQt6.QtWidgets.QToolButton`) for these buttons.
...
A variation of a command button is a menu button. These provide not just one command, but several, since when they are clicked they pop up a menu of options. Use the method :sip:ref:`~PyQt6.QtWidgets.QPushButton.setMenu` to associate a popup menu with a push button.
