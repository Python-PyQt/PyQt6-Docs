.. sip:method-description::
    ...
    :digest: 23ec69f08c7e42663a1dc20f0d700a9b
...
**Note:** On embedded platforms, setFocus() will not cause an input panel to be opened by the input method. If you want this to happen, you have to send a :sip:ref:`~PyQt6.QtCore.QEvent.Type.RequestSoftwareInputPanel` event to the widget yourself.

setFocus() gives focus to a widget regardless of its focus policy, but does not clear any keyboard grab (see :sip:ref:`~PyQt6.QtWidgets.QWidget.grabKeyboard`).

Be aware that if the widget is hidden, it will not accept focus until it is shown.

**Warning:** If you call setFocus() in a function which may itself be called from :sip:ref:`~PyQt6.QtWidgets.QWidget.focusOutEvent` or :sip:ref:`~PyQt6.QtWidgets.QWidget.focusInEvent`, you may get an infinite recursion.
