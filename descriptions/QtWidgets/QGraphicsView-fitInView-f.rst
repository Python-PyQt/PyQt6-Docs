.. sip:method-description::
    ...
    :digest: 3070c6512b77081ed2667ac8b506226a

Scales the view matrix and scrolls the scroll bars to ensure that the scene rectangle *rect* fits inside the viewport. *rect* must be inside the scene rect; otherwise,  cannot guarantee that the whole rect is visible.
...
It's common to call  from inside a reimplementation of :sip:ref:`~PyQt6.QtWidgets.QGraphicsView.resizeEvent`, to ensure that the whole scene, or parts of the scene, scales automatically to fit the new size of the viewport as the view is resized. Note though, that calling  from inside :sip:ref:`~PyQt6.QtWidgets.QGraphicsView.resizeEvent` can lead to unwanted resize recursion, if the new transformation toggles the automatic state of the scrollbars. You can toggle the scrollbar policies to always on or always off to prevent this (see horizontalScrollBarPolicy() and verticalScrollBarPolicy()).
