.. sip:method-description::
    :status: todo
    :pysig: de51477325cca642db4f9cb322a9317b
...
The *localPos* is the mouse cursor's position relative to the receiving widget or item. The cursor's position in screen coordinates is specified by *globalPos*. The window position is set to the same value as *localPos*. The *button* that caused the event is given as a value from the :sip:ref:`~PyQt6.QtCore.Qt.MouseButton` enum. If the event *type* is :sip:ref:`~PyQt6.QtCore.QEvent.Type.MouseMove`, the appropriate button for this event is :sip:ref:`~PyQt6.QtCore.Qt.MouseButton.NoButton`. *buttons* is the state of all buttons at the time of the event, *modifiers* the state of all keyboard modifiers.
