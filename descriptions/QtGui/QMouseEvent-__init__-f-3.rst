...
The *localPos* is the mouse cursor's position relative to the receiving widget or item. The window position is set to the same value as *localPos*. The *button* that caused the event is given as a value from the :sip:ref:`~PyQt6.QtCore.Qt.MouseButton` enum. If the event *type* is :sip:ref:`~PyQt6.QtCore.QEvent.Type.MouseMove`, the appropriate button for this event is :sip:ref:`~PyQt6.QtCore.Qt.MouseButton.NoButton`. The mouse and keyboard states at the time of the event are specified by *buttons* and *modifiers*.
