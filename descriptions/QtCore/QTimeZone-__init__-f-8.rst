...
The *offsetSeconds* from UTC must be in the range -16 hours to +16 hours. The *zoneId* *must not* be an ID for which :sip:ref:`~PyQt6.QtCore.QTimeZone.isTimeZoneIdAvailable` is true, unless it is a UTC-offset name that doesn't appear in :sip:ref:`~PyQt6.QtCore.QTimeZone.availableTimeZoneIds`.
