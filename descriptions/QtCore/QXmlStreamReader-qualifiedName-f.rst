...
A qualified name is the raw name of an element in the XML data. It consists of the namespace prefix, followed by colon, followed by the element's local name. Since the namespace prefix is not unique (the same prefix can point to different namespaces and different prefixes can point to the same namespace), you shouldn't use , but the resolved :sip:ref:`~PyQt6.QtCore.QXmlStreamReader.namespaceUri` and the attribute's local :sip:ref:`~PyQt6.QtCore.QXmlStreamReader.name`.
