.. sip:method-description::
    ...
    :realsig: ()
    :digest: 6c1285d1865a01aad7ae9378948f4d4e

Unloads the plugin and returns ``true`` if the plugin could be unloaded; otherwise returns ``false``.

This happens automatically on application termination, so you shouldn't normally need to call this function.

If other instances of :sip:ref:`~PyQt6.QtCore.QPluginLoader` are using the same plugin, the call will fail, and unloading will only happen when every instance has called .
