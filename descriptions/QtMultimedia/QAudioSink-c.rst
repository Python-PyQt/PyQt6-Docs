.. sip:class-description::
    ...
    :digest: 947db72286975c1def6d389212cc9b6c
...
If an error occurs, you can fetch the error type with the :sip:ref:`~PyQt6.QtMultimedia.QAudioSink.error` function. Please see the QtAudio::Error enum for a description of the possible errors that are reported. When QtAudio::UnderrunError is encountered, the state changes to QtAudio::IdleState, when another error is encountered, the state changes to QtAudio::StoppedState. You can check for errors by connecting to the :sip:ref:`~PyQt6.QtMultimedia.QAudioSink.stateChanged` signal:
